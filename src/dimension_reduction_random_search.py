...
def stochastic_solver(function_instance, n, seed=0,torch_seed=0,**kwargs): 
    ...
    # now, we have the subspace. just perform the random search on this domain.
    # directions is the plane
    # domain in m-dimensional space is [alpha_extent[0].reshape(1,-1), alpha_extent[1].reshape(1,-1)]
    
    
    points = rng.uniform(alpha_extent[0], alpha_extent[1], size =(n,m))
    ...
    for x in points:
        value = oracle(directions @ x).item()
