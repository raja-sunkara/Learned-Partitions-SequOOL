...
def oo(function_instance, n, seed=0, **kwargs):
    ...
    for i in range(settings['h_max']):
        t[i]['x_max'] = np.empty((0, d), dtype=float)
        t[i]['x_min'] = np.empty((0, d), dtype=float)
        t[i]['x'] = np.empty((0, d), dtype=float)
        t[i]['leaf'] = []
        t[i]['new'] = []
        t[i]['cen_val'] = []
    ...
    try:
        t[0]['cen_val'] = [oracle(t[0]['x']).item()] #[f(t[0]['x'][0,:])]
    except:
        t[0]['cen_val'] = [f(t[0]['x'])]
    ...
    at_least_one = 1
    n = 1
    regret.append(fmax-t[0]['cen_val'][0])
                ...
                if (h + 1 > settings['h_max']):
                    ...
                    if (b_hi_max >= v_max):
                        ...
                        if settings['sample_when_created']:
                            try:
                                sampled_value = oracle(x_g).item() #f(x_g)
                            except:
                                sampled_value = f(x_g.reshape(1,-1))
                        ...
                        if settings['sample_when_created']:
                            ...
                            try:
                                sampled_value = oracle(x_d).item()#f(x_d)
                            except:
                                sampled_value = f(x_d.reshape(1,-1))
                            # sampled_value = f(x_d)
                            if sampled_value > finaly:
                                finalx = x_d
                                #### --------------------------
